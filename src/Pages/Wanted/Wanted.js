...
class Wanted extends React.Component {
  ...
  componentDidMount() {
    ...
    if (user && user.cityId && user && user.role ==='4') {
      ...
      this.props.dispatch(professionalAction.getNews(3));
  ...
  onNewsStatusChange = (values,flag) =>{
    if(values){
      ...
      message.success('Update the Wanted status !!!')
    }
  }
  ...
  onFormFinish =  (values) => {
    ...
    if(values && parseInt(values.newsType) !== 1){
      ...
      message.success('Wanted save Successfully !!!');
    }
...
const connectedWanted = connect(mapStateToProps)(Wanted);
export { connectedWanted as Wanted };
